...
from audit_submissions import audit, valid_repository
...
def _load_entries(path: Path) -> list[dict[str, object]]:
    ...
    if isinstance(payload, list):
        ...
        raise TypeError("输入必须是数组，或包含 submissions/files 数组的 JSON 对象")
...
def prepare(
    ...
    *,
    ...
    if not valid_repository(repository) or license_name != "CC BY 4.0":
        raise ValueError(
            "repository 必须是 GitHub owner/repo，license 必须为 CC BY 4.0"
        )
    ...
    for index, entry in enumerate(entries, start=1):
        ...
        if not isinstance(raw_path, str) or not raw_path.strip():
            ...
            continue
        source = (input_path.parent / Path(raw_path).expanduser()).resolve()
        ...
        for field in ("university", "school"):
            if (
                field in entry
                and str(entry[field]).strip() != str(summary[field]).strip()
            ):
        ...
        prepared.append(
            {
                ...
                "department": str(
                    entry.get(
                        "department", departments[0] if len(departments) == 1 else ""
                    )
                ),
                "source": source,
                "sha256": file_hash,
    ...
    if errors:
        raise ValueError(f"{len(errors)} 项校验错误：" + "；".join(errors[:10]))
    prepared.sort(
        key=lambda item: (
            str(item["university"]).casefold(),
            str(item["school"]).casefold(),
            str(item["department"]).casefold(),
            str(item["sha256"]),
        )
    )
    ...
    result = {
        ...
        "totals": {
            ...
            "professor_count": sum(int(item["professor_count"]) for item in items),
        },
        "submission": {
            "status": "prepared",
            "issue_urls": [],
    ...
    if not dry_run:
        ...
        if output_dir.exists() and any(output_dir.iterdir()):
            raise ValueError(
                f"输出目录非空：{output_dir}；请换用空目录，或审计并继续已有批次"
            )
        ...
        try:
            ...
            for index, item in enumerate(prepared, start=1):
                shutil.copyfile(
                    item["source"], temporary / "files" / f"{index:03d}.xlsx"
                )
            (temporary / "manifest.json").write_text(
                json.dumps(result, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
            copied_audit = audit(temporary / "manifest.json")
            if not copied_audit["ok"]:
                raise ValueError("复制后审计失败：" + "；".join(copied_audit["errors"]))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--input", type=Path, required=True, help="包含 submissions 数组的 JSON"
    )
    ...
    parser.add_argument("--details", action="store_true", help="额外输出完整 manifest")
            ...
            json.dumps(
                {
                    ...
                    "code": "PREPARE_FAILED",
                    "errors": [str(exc)],
                    "next_action": "修复输入后重新准备；已有批次使用 audit/submit",
                },
                ensure_ascii=False,
                indent=2,
            )
    ...
    output = {
        "ok": True,
        "status": "planned" if args.dry_run else "prepared",
        ...
        "manifest": None
        if args.dry_run
        else str(args.output_dir.expanduser().resolve() / "manifest.json"),
        "next_action": "prepare_without_dry_run" if args.dry_run else "submit_plan",
    }
    if args.details:
