...
def _batch_id(items: list[dict[str, object]]) -> str:
    canonical = [{key: item[key] for key in ("university", "school", "department", "file", "sha256", "professor_count")} for item in items]
...
def audit(manifest_path: Path) -> dict[str, object]:
    ...
    if not isinstance(manifest, dict) or manifest.get("schema_version") != 1:
        return {"ok": False, "manifest": str(manifest_path), "errors": ["manifest schema_version 必须为 1"]}
    ...
    for position, item in enumerate(items, start=1):
        ...
        identity = (str(expected["university"]).casefold(), str(expected["school"]).casefold())
        if identity in identities:
            errors.append(f"重复投稿单位：{expected['university']} / {expected['school']}")
        identities.add(identity)
        normalized.append(expected)
    recomputed = _batch_id(normalized) if normalized else ""
    if manifest.get("batch_id") != recomputed:
        errors.append(f"batch_id 不匹配：manifest={manifest.get('batch_id')!r} recomputed={recomputed!r}")
    totals = manifest.get("totals")
    if isinstance(totals, dict):
        if totals.get("submission_count") != len(normalized):
            errors.append("totals.submission_count 不匹配")
        if totals.get("professor_count") != sum(int(item["professor_count"]) for item in normalized):
            errors.append("totals.professor_count 不匹配")
    else:
        errors.append("manifest.totals 必须是对象")
    submission = manifest.get("submission")
    if not isinstance(submission, dict) or submission.get("status") not in {"prepared", "planned", "submitted", "unknown", "failed", "verified"}:
        errors.append("submission.status 不是支持的状态")
    return {"ok": not errors, "manifest": str(manifest_path), "batch_id": recomputed, "item_count": len(normalized), "professor_count": sum(int(item["professor_count"]) for item in normalized), "status": submission.get("status") if isinstance(submission, dict) else None, "errors": errors}
