...
G1–G10 全部通过；解析错误、发现闭环、计划语义、构建身份、安装诊断、GUI 动作级覆盖与流式导出加固也已纳入同一套自动化门禁。
...
| 目标 | 结果 | 自动化证据 |
| --- | --- | --- |
| G1 命令合同覆盖率 | 147/147（100%） | `test_every_available_leaf_has_a_complete_schema_validated_contract`；每个可用叶子命令都有输入、输出、效果、前置条件、错误和后续动作合同。 |
| G2 发现链路闭环 | 147/147；元数据漂移 0 | `test_versioned_baseline_lists_match_the_live_capability_registry`、`test_capabilities_and_describe_have_zero_metadata_drift`、`test_next_actions_only_reference_real_commands_or_generic_wait`。 |
...
| 范围 | 命令 | 结果 |
| --- | --- | --- |
| CLI | `PYTHONPATH=cli/src python -m unittest discover -s cli/test` | 137 tests，全部通过 |
...
| 桌面端 | `cd desktop && npm run test` | 14 files / 122 tests，全部通过 |
