...
- 业务命令要求用户先手动打开桌面软件；当前命令和能力以 CLI 自描述输出为准。

> 已完成的接口演进与后续设计说明见 [Agent-first 通用 CLI 演进路线](../development/agent_cli_evolution_plan.md)。该路线补充本文的实现阶段，不改变本文已经确认的安全和产品边界。
...
20. 软件未运行时，CLI 不启动或唤醒桌面应用；业务命令应明确提示用户先手动打开软件。
...
用户需要先手动打开 Auto Email Sender，并等待本地服务加载完成。

业务命令的行为：

1. CLI 查找当前桌面应用的本地运行信息。
2. 如果应用已经运行，直接连接。
3. 如果应用未运行，返回 `APP_UNAVAILABLE`，提示用户手动打开软件后重试。
4. 如果应用正在启动但本地服务尚未就绪，提示用户等待加载完成后重试。
5. CLI 不会创建桌面进程、隐藏窗口或等待后台唤醒。
6. 如果必须由用户完成邮箱配置等操作，再提示用户在桌面端完成。

`version`、`--help`、`guide`、`status` 和 `doctor` 不会启动桌面应用；业务命令只在已运行的本地服务上执行。

### 7.2 Electron 需要增加的行为

当前应用已经使用单实例和托盘。Agent 支持需要：

- 后端准备完成后写入运行描述文件。
- 应用退出时删除自己持有的运行描述文件。

长时间运行的抓取、匹配或发送任务需要后台 worker。用户可继续通过托盘退出应用；CLI 不会替用户启动应用。
...
JSON `invoke` 的位置参数始终按数据解析，包括以 `--` 开头的字符串；输入文件使用 UTF-8，编码错误返回结构化错误。全局选项可放在命令组与子命令之间，叶子命令的同名选项仍按其自身合同解释。

字节预算计入业务 `data` 内的投影和续取元数据。集合截断后的 `recovery_action` 同时提供 `output_file` 和 `input.all_items=true`：保留原查询条件与起始游标，覆盖这些恢复参数后逐页导出。单独使用 `--output-file` 仍只导出当前请求的分页范围。
...
产品分发的 Skill 不写入任何用户机器的绝对路径。CLI 使用稳定的标准命令名；`installation.json` 只用于记录产品管理的 CLI 和 Skill 安装状态，不能用于定位或启动桌面程序；macOS 命令入口则链接到应用包内的 CLI。这样既不会污染仓库中的 Skill，也不会把某一台机器的路径带给其他用户。
...
命令、字段和能力状态以 CLI 的 `capabilities`、`describe` 输出为准。本文只维护产品决策与必要的设计原因，不复制当前命令计数或为常规修改新增验收报告。
