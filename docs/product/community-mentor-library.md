...
2. 用户按学校和学院选择需要的分片，一次最多 20 个学院、2,000 位导师；页面会在勾选时提前显示并阻止超限组合。
3. 应用在本地完成姓名、全部邮箱、全部当前任职、研究方向和状态搜索；导师卡片每页渲染 100 位。
4. 用户选择导师并生成导入预览。
5. 新记录默认新增；本地空字段默认由社区补全；本地非空字段默认保留。
6. 本地和社区同时修改的字段逐项选择后再提交；社区空值默认保留本地，只有用户明确选择采用社区空值时才清空对应内容。
7. 每次预览都会绑定当时的本地数据状态。如果用户在预览后又编辑了导师，旧预览会失效并要求重新预览，避免覆盖刚完成的修改。

标签、个人备注、任务、发送记录、通信历史和匹配结果不会被下载、上传或覆盖。

### 贡献和反馈

- 社区导师库页面以“批量贡献学校/学院”为主入口：进入导师管理后按学校或学院筛选，选择全部筛选结果，再点击“贡献到社区”。
- “贡献到社区”会先通过系统保存窗口写入仓库批量投稿器可直接读取的普通 XLSX，确认保存成功后才用系统浏览器打开 GitHub 批量投稿表；取消保存不会打开浏览器。表头严格为社区安全字段，不包含公式、宏、标签或本地私有数据。
...
- 单文件、单次学院数量、导师总数和总下载大小均有限制；
- Catalog、Manifest 和分片的 ID、路径、学校/学院归属、版本、生成时间和记录数必须互相一致；
- `latest.json` 与其他核心文件一起保存在版本目录中，只有四个核心文件全部验证成功后才原子更新当前缓存索引；
- 网络失败时只回退到最后一次完整验证成功的缓存；
- 缓存会自动保留当前版本和最近版本，并将总占用控制在约 500 MB；空间不足时优先清理旧版本和最久未使用的学院分片，需要时会重新下载，用户无需手动维护。
