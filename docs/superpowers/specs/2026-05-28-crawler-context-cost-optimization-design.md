...
由于 DeepAgents 内部 messages 管理细节由框架封装，直接删除历史消息风险较高。更稳妥的方式是在后端运行时设置单次 Agent 运行边界：只在已完成一定数量 chunk、且数据库中仍有可恢复的待处理 work 时，让本次 Agent 自然结束，再由现有任务运行时重新启动下一轮。
...
第一版只实现“完成 chunk 数上限”，并且只有在 chunk 已完整提交或标记完成、且数据库中仍存在可恢复的待处理 work 时才停止，避免丢失 Agent 刚发现但尚未持久化的新页面线索。

### 重新调度方式

当单次 Agent 到达边界时，不把任务标记为完成，而是返回“本轮已达到上下文成本边界”。任务运行时检查数据库中是否仍存在待处理 chunk 或可继续探索的工作。成本优化不得中断未完成工具调用、未提交 chunk、未持久化的新发现页面线索：
...
默认值应偏保守，但不得优先于正常抓取流程；后续调整必须验证不会中断未持久化的新页面线索。
