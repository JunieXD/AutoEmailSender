...
| 首次 overlap | 候选递归 overlap | LLM 调用节点 | 重复候选 | 唯一候选 |
| ---: | ---: | ---: | ---: | ---: |
| 180 | 30 | 48 | 48 | 150 |
| 180 | 15 | 36 | 20 | 150 |
| 180 | 10 | 36 | 20 | 150 |
...
- 调用节点从 48 降到 36，减少 25%；
- 重复候选从 48 降到 20；
- 150 位唯一候选无遗漏。

15 和 10 在该页面上的结果相同，因此选择 15，为跨 chunk 边界的姓名和链接保留更多上下文余量。
...
- 对同一华科快照重放后仍发现 150 个唯一教师链接。
- 候选递归 overlap 为 15 时，重复候选不高于 20，调用节点不高于 36。
...
- 华科固定快照回归得到 150 个唯一候选，调用节点不高于 36，重复候选不高于 20。
