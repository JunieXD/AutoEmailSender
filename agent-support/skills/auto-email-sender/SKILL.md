...
1. Start with `auto-email-sender --format json capabilities`. It returns a compact resource catalog. Narrow with `capabilities --resource <resource>`, then inspect a selected command with `describe --command <command>`.
