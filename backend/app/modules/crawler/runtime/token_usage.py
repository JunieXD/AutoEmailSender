...
async def record_crawler_token_usage(
    ...
    *,
    ...
    claim: CrawlerClaimFence | None = None,
) -> bool:
    kind = (
    ...
    async with session_factory() as session:
        if claim is not None and not await fence_crawler_claim(session, claim):
        ...
        if claim is not None:
            existing = await session.scalar(
                select(CrawlWorkerTokenUsage.id).where(
                    CrawlWorkerTokenUsage.run_id == run.id,
                    CrawlWorkerTokenUsage.worker_kind == kind,
                    CrawlWorkerTokenUsage.work_item_id == str(work_item_id),
                    CrawlWorkerTokenUsage.claim_id == claim.worker_id,
                )
            )
        ...
        session.add(
            CrawlWorkerTokenUsage(
                ...
                claim_id=claim.worker_id if claim is not None else None,
