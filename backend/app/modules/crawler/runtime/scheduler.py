...
async def claim_next_work(
    ...
    *,
    ...
    async with _CLAIM_LOCK:
        return await _claim_next_work_locked(
            session_factory,
            worker_id=worker_id,
            config=config,
        )
...
async def _claim_next_work_locked(
    session_factory: async_sessionmaker[AsyncSession],
    *,
    worker_id: str,
    config: CrawlerWorkerConfig | None,
) -> CrawlerClaimedWork:
    config = config or CrawlerWorkerConfig()
    async with session_factory() as session:
        ...
        config = CrawlerWorkerConfig(
            ...
            enrichment_concurrency=max(
                1,
                int(
                    runtime_settings.crawler_profile_enrichment_concurrency
                    or config.enrichment_concurrency
                ),
            ),
            enrichment_host_concurrency=max(
                1,
                int(
                    runtime_settings.crawler_host_concurrency
                    or config.enrichment_host_concurrency
                ),
            ),
        ...
        for job_id in job_ids:
            ...
            if claimed.kind is CrawlerWorkKind.IDLE:
            ...
            if claimed.kind is not CrawlerWorkKind.IDLE:
                ...
                return claimed
        await session.commit()
        return CrawlerClaimedWork.idle()
    ...
    *,
    ...
    running_jobs = list(
        await session.scalars(
            ...
            .order_by(
                CrawlJob.updated_at.asc(), CrawlJob.created_at.asc(), CrawlJob.id.asc()
            )
        )
    )
...
async def finalize_idle_jobs(session: AsyncSession) -> None:
    ...
    for job in jobs:
        ...
        if not has_candidates:
            ...
            error_message = (
                await _job_terminal_failure_message(session, job_id=job.id)
                or "抓取未发现候选导师"
            )
        else:
        ...
        await mark_crawl_job_run_finished(
            session, job, status=final_status, error_message=error_message, now=now
        )
...
async def _claim_page_task(
    ...
    *,
    ...
    if task is None:
        return CrawlerClaimedWork.idle()
    result = await _conditional_claim_page_task(
        session,
        task=task,
        worker_id=worker_id,
        now=now,
    ...
    return CrawlerClaimedWork(
        kind=CrawlerWorkKind.PAGE, work_item_id=task.id, job_id=task.job_id
    )
...
async def _claim_chunk(
    ...
    *,
    ...
    if chunk is None:
        return CrawlerClaimedWork.idle()
    result = await _conditional_claim_chunk(
        session,
        chunk=chunk,
        worker_id=worker_id,
        now=now,
    ...
    return CrawlerClaimedWork(
        kind=CrawlerWorkKind.CHUNK, work_item_id=chunk.id, job_id=chunk.job_id
    )
...
async def _claim_enrichment_task(
    ...
    *,
    ...
    if task is None:
        return CrawlerClaimedWork.idle()
    result = await _conditional_claim_enrichment_task(
        session,
        task=task,
        worker_id=worker_id,
        now=now,
    ...
    await _mark_job_running_for_claimed_work(session, job_id=task.job_id, now=now)
    return CrawlerClaimedWork(
        kind=CrawlerWorkKind.ENRICHMENT, work_item_id=task.id, job_id=task.job_id
    )


async def _active_enrichment_host_counts(
...
async def _mark_job_running_for_claimed_work(
    session: AsyncSession, *, job_id: int, now: datetime
) -> None:
    job = await session.get(CrawlJob, job_id)
    if job is None:
        return
...
async def _conditional_claim_enrichment_task(
    ...
    *,
    ...
    return await session.execute(
        ...
        .execution_options(synchronize_session=False)
        .where(
            CrawlCandidateEnrichmentTask.id == task.id, _enrichment_task_claimable(now)
        )
        .values(
...
async def _job_has_available_or_leased_work(
    session: AsyncSession, *, job_id: int, now: datetime
) -> bool:
    ...
    *,
    ...
    for candidate_id, status in rows:
        ...
        if (
            ...
            or status_priority[status] > status_priority[current_status]
        ):
...
async def _job_terminal_failure_message(
    ...
    enrichment_errors = await session.execute(
        select(
            CrawlCandidateEnrichmentTask.updated_at,
            CrawlCandidateEnrichmentTask.last_error,
        ).where(
            CrawlCandidateEnrichmentTask.job_id == job_id,
            CrawlCandidateEnrichmentTask.status
            == CrawlCandidateEnrichmentTaskStatus.FAILED_TERMINAL.value,
...
async def run_crawler_once(
    session_factory: async_sessionmaker[AsyncSession],
    *,
    worker_id: str = "crawler-worker",
    config: CrawlerWorkerConfig | None = None,
) -> int:
    resolved_config = config or CrawlerWorkerConfig()
    ...
    if claimed.work_item_id is None:
        return 0
    work: Awaitable[int]
    if claimed.kind is CrawlerWorkKind.PAGE:
        from .page_worker import run_crawler_page_worker_once

        work = run_crawler_page_worker_once(
            session_factory,
            task_id=claimed.work_item_id,
            worker_id=claim_owner_id,
        )
    elif claimed.kind is CrawlerWorkKind.CHUNK:
        from .chunk_worker import run_crawler_chunk_worker_once

        work = run_crawler_chunk_worker_once(
            session_factory,
            chunk_id=claimed.work_item_id,
            worker_id=claim_owner_id,
        )
    elif claimed.kind is CrawlerWorkKind.ENRICHMENT:
        from .enrichment_worker import run_crawler_enrichment_worker_once

        work = run_crawler_enrichment_worker_once(
            session_factory,
            task_id=claimed.work_item_id,
            worker_id=claim_owner_id,
        )
...
async def _run_claim_heartbeat(
    ...
    *,
    ...
    while True:
        ...
        try:
            if not await renew_crawler_claim(
                session_factory,
                claim,
                lease_seconds=lease_seconds,
            ):
