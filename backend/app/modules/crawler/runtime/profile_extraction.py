...
class ProfileExtractionPayload(BaseModel):
    status: str = "no_candidate"
    candidate: dict[str, Any] | None = None


@dataclass(frozen=True)
class ProfileExtractionAttempt:
    attempt_number: int
    raw_model_text: str
...
class ProfileExtractionResult:
    ...
    attempts: list[ProfileExtractionAttempt] = field(default_factory=list)
    page_text_hash: str | None = None
    page_text_length: int = 0


async def invoke_profile_extraction_agent(
    llm_profile: Any,
    *,
    ...
    base_prompt = build_profile_extraction_prompt(
    ...
    (
        ...
        adaptation,
        prompt=base_prompt,
        result_model=CrawlerProfileExtractionWirePayload,
    )
    parsed = ProfileExtractionPayload.model_validate(
        profile_wire_to_dict(wire_payload)
    ).model_dump()
...
def build_profile_extraction_prompt(
    *,
    ...
    return (
        "你是 AutoEmailSender 的详情页整页抽取 Worker。只处理当前页面，不要发现新 URL，不要请求新页面。\n"
