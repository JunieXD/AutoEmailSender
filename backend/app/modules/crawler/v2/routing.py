...
def build_v2_entry_routing_prompt(
    *,
    ...
    return (
        ...
        "如果当前页没有直接列出多人，只选择能直接到达目标名单的最少必要页面或承载主要名单的 iframe。\n"
        "如果可见正文已经列出多人，无论姓名能否点击都算名单；只有同一目标人群被明确分成并列分类、当前页只是其中一类时，才选择尚未覆盖的同组分类。"
        "这里的分类绝不包括同一名单的前后页或页码，分页由另一阶段处理。"
        "这种互补切分可以基于人员类别或单位内部组织，按页面实际含义判断，不依赖固定栏目名称。"
        "同一批人的另一种排序或重复分组不是缺失部分；已不在当前单位常规任职范围的人也不纳入。"
        "如果存在多套分类方式，只选择一套覆盖较完整、彼此重叠较少且页面较少的分类，不要混选多套。\n"
        "如果当前页已经是完整名单，或没有明确的同级互补名单，返回空数组。\n"
        "不要选择当前页、个人主页、分页、返回网站首页或上级目录、下属单位主页、宽泛导航、用途不同的栏目、登录区、文件或站外页面。\n"
