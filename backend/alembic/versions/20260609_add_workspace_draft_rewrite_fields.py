...
def upgrade() -> None:
    with op.batch_alter_table("email_tasks", schema=None) as batch_op:
        batch_op.add_column(sa.Column("draft_generation_started_at", sa.DateTime(timezone=True), nullable=True))
        batch_op.add_column(sa.Column("draft_rewrite_source_subject", sa.Text(), nullable=True))
        batch_op.add_column(sa.Column("draft_rewrite_source_body_text", sa.Text(), nullable=True))
        batch_op.add_column(sa.Column("draft_rewrite_source_body_html", sa.Text(), nullable=True))
        batch_op.add_column(sa.Column("draft_rewrite_source_selected_material_ids", sa.JSON(), nullable=True))
