...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "transient_failure_count", sa.Integer(), server_default="0", nullable=False
        ),
        ...
        sa.ForeignKeyConstraint(
            ["last_page_id"], ["crawl_pages.id"], ondelete="SET NULL"
        ),
    ...
    op.create_index(
        "ix_crawl_page_fetch_states_job_id", "crawl_page_fetch_states", ["job_id"]
    )
    op.create_index(
        "ix_crawl_page_fetch_states_status", "crawl_page_fetch_states", ["status"]
    )


def downgrade() -> None:
    op.drop_index(
        "ix_crawl_page_fetch_states_status", table_name="crawl_page_fetch_states"
    )
    op.drop_index(
        "ix_crawl_page_fetch_states_job_id", table_name="crawl_page_fetch_states"
    )
