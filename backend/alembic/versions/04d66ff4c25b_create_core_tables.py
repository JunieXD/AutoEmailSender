...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "smtp_port", sa.Integer(), server_default=sa.text("(465)"), nullable=False
        ),
        ...
        sa.UniqueConstraint(
            "email_address", name=op.f("uq_identity_profiles_email_address")
        ),
    )
    op.create_table(
        "llm_profiles",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            name=op.f("fk_attachment_assets_identity_id_identity_profiles"),
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "is_replied", sa.Boolean(), server_default=sa.text("0"), nullable=False
        ),
        sa.Column("last_error", sa.Text(), nullable=True),
        sa.ForeignKeyConstraint(
            ["identity_id"],
            ["identity_profiles.id"],
            name=op.f("fk_email_tasks_identity_id_identity_profiles"),
        ),
        sa.ForeignKeyConstraint(
            ["llm_profile_id"],
            ["llm_profiles.id"],
            name=op.f("fk_email_tasks_llm_profile_id_llm_profiles"),
        ),
        sa.ForeignKeyConstraint(
            ["professor_id"],
            ["professors.id"],
            name=op.f("fk_email_tasks_professor_id_professors"),
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_email_tasks")),
    )
    op.create_index(
        op.f("ix_email_tasks_identity_id"), "email_tasks", ["identity_id"], unique=False
    )
    op.create_index(
        op.f("ix_email_tasks_llm_profile_id"),
        "email_tasks",
        ["llm_profile_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_email_tasks_professor_id"),
        "email_tasks",
        ["professor_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_attachment_assets_identity_id"), table_name="attachment_assets"
    )
