...
def _create_delivery_attempts_table() -> None:
    if "email_delivery_attempts" in _table_names():
        _upgrade_split_beta_delivery_attempts_table()
        return
    op.create_table(
...
def _upgrade_split_beta_delivery_attempts_table() -> None:
    columns = _column_names("email_delivery_attempts")
    if "attempt_id" not in columns:
        return

    for index_name in _index_names("email_delivery_attempts"):
        op.drop_index(index_name, table_name="email_delivery_attempts")

    email_task_fk = next(
        (
            foreign_key
            for foreign_key in _foreign_keys("email_delivery_attempts")
            if foreign_key.get("constrained_columns") == ["email_task_id"]
        ),
        None,
    )
    with op.batch_alter_table(
        ...
        if email_task_fk is not None:
            batch_op.drop_constraint(
                str(email_task_fk.get("name") or "fk_email_delivery_attempts_email_task_id_email_tasks"),
                type_="foreignkey",
            )
        ...
        batch_op.add_column(sa.Column("subject_fingerprint", sa.String(length=71), nullable=True))
        batch_op.add_column(sa.Column("content_fingerprint", sa.String(length=71), nullable=True))
        batch_op.add_column(sa.Column("app_message_id", sa.String(length=255), nullable=True))
        batch_op.add_column(
            sa.Column("normalized_app_message_id", sa.String(length=255), nullable=True),
        )
        batch_op.add_column(
        ...
        batch_op.create_foreign_key(
            "fk_email_delivery_attempts_email_task_id_email_tasks",
            "email_tasks",
            ["email_task_id"],
            ["id"],
            ondelete="SET NULL",
        ...
        batch_op.create_foreign_key(
            "fk_email_delivery_attempts_identity_id_identity_profiles",
            "identity_profiles",
            ["identity_id"],
            ["id"],
            ondelete="CASCADE",
        ...
        batch_op.create_foreign_key(
            "fk_email_delivery_attempts_professor_id_professors",
            "professors",
            ["professor_id"],
            ["id"],
            ondelete="CASCADE",
    ...
    bind.execute(
        sa.text(
            ...
            SET identity_id = (
                    SELECT identity_id FROM email_tasks
                    WHERE email_tasks.id = email_delivery_attempts.email_task_id
                ),
                professor_id = (
                    SELECT professor_id FROM email_tasks
                    WHERE email_tasks.id = email_delivery_attempts.email_task_id
                ),
                recipient_email = COALESCE((
                    SELECT professors.email
                    FROM email_tasks
                    JOIN professors ON professors.id = email_tasks.professor_id
                    WHERE email_tasks.id = email_delivery_attempts.email_task_id
                ), ''),
                attempt_number = (
                    SELECT COUNT(*)
                    FROM email_delivery_attempts AS prior
                    WHERE prior.email_task_id = email_delivery_attempts.email_task_id
                      AND (
                        prior.started_at < email_delivery_attempts.started_at
                        OR (
                          prior.started_at = email_delivery_attempts.started_at
                          AND prior.id <= email_delivery_attempts.id
                        )
                      )
                ),
                app_message_id = prepared_rfc_message_id,
                normalized_app_message_id = lower(prepared_rfc_message_id),
                status = CASE outcome
                    WHEN 'smtp_accepted' THEN 'accepted'
                    WHEN 'pre_submission_failed' THEN 'failed'
                    WHEN 'assumed_sent_after_interruption' THEN 'unknown'
                    ELSE 'prepared'
                END,
    ...
    rows = bind.execute(
        sa.text("SELECT id, subject, content FROM email_delivery_attempts"),
    ).mappings()
    for row in rows:
        bind.execute(
            sa.text(
                """
                UPDATE email_delivery_attempts
                SET subject_fingerprint = :subject_fingerprint,
    ...
    with op.batch_alter_table(
        ...
        for column_name, column_type in (
            ("identity_id", sa.Integer()),
            ("professor_id", sa.Integer()),
            ("attempt_number", sa.Integer()),
            ("recipient_email", sa.String(length=255)),
            ("subject_fingerprint", sa.String(length=71)),
            ("content_fingerprint", sa.String(length=71)),
        ):
            batch_op.alter_column(
                column_name,
        ...
        batch_op.create_unique_constraint(
            "uq_email_delivery_attempts_task_number",
            ["email_task_id", "attempt_number"],
        )

    for index_name, columns_to_index in (
        ("ix_email_delivery_attempts_email_task_id", ["email_task_id"]),
        ("ix_email_delivery_attempts_identity_id", ["identity_id"]),
        ("ix_email_delivery_attempts_professor_id", ["professor_id"]),
        (
            "ix_email_delivery_attempts_identity_professor_started",
            ["identity_id", "professor_id", "started_at", "id"],
        ),
        ...
        op.create_index(index_name, "email_delivery_attempts", columns_to_index)
...
def _add_email_log_reconciliation_columns() -> None:
    ...
    delivery_attempt_fk = next(
        (
            foreign_key
            for foreign_key in _foreign_keys("email_logs")
            if foreign_key.get("constrained_columns") == ["delivery_attempt_id"]
        ),
        None,
    )
    needs_delivery_attempt_fk_rebuild = bool(
        not needs_delivery_attempt
        and (
            delivery_attempt_fk is None
            or delivery_attempt_fk.get("referred_columns") != ["id"]
        )
    )
    with op.batch_alter_table(
        "email_logs",
        ...
        if needs_delivery_attempt_fk_rebuild and delivery_attempt_fk is not None:
            batch_op.drop_constraint(
                str(
                    delivery_attempt_fk.get("name")
                    or "fk_email_logs_delivery_attempt_id_email_delivery_attempts"
                ),
                type_="foreignkey",
            )
        ...
        if needs_delivery_attempt or needs_delivery_attempt_fk_rebuild:
            batch_op.create_foreign_key(
...
def downgrade() -> None:
    ...
    if "email_logs" in tables:
        columns = _column_names("email_logs")
        indexes = _index_names("email_logs")
        foreign_keys = _foreign_keys("email_logs")
        delivery_attempt_fk = next(
            (
                foreign_key
        ...
        merged_into_fk = next(
            (
                foreign_key
                for foreign_key in foreign_keys
                if foreign_key.get("constrained_columns") == ["merged_into_id"]
            ),
            None,
        )
        ...
        with op.batch_alter_table(
            ...
            if "delivery_attempt_id" in columns:
                if delivery_attempt_fk is not None:
                    batch_op.drop_constraint(
                        str(
                            delivery_attempt_fk.get("name")
                            or "fk_email_logs_delivery_attempt_id_email_delivery_attempts"
            ...
            if "merged_into_id" in columns:
                if merged_into_fk is not None:
                    batch_op.drop_constraint(
                        str(
                            merged_into_fk.get("name")
                            or "fk_email_logs_merged_into_id_email_logs"
                        ),
                        type_="foreignkey",
                    )
