...
class MigrationScriptTests(unittest.TestCase):
    ...
    def test_master_reconciliation_upgrades_an_already_applied_split_beta_schema(
        self,
    ) -> None:
        database_path = Path(self.temp_dir.name) / "applied-split-beta.db"
        env = os.environ.copy()
        env["DATABASE_URL"] = f"sqlite+aiosqlite:///{database_path.as_posix()}"
        self._run_alembic(env, "upgrade", "20260810_agent_ui_handoffs")

        connection = sqlite3.connect(database_path)
        connection.execute("PRAGMA foreign_keys = ON")
        identity_id = DatabaseSchemaTests._insert_identity_into(
            connection,
            email_address="split-beta-upgrade@example.com",
        )
        llm_profile_id = DatabaseSchemaTests._insert_llm_profile_into(
            connection,
            name="Split beta upgrade model",
        )
        professor_id = DatabaseSchemaTests._insert_professor_into(
            connection,
            "split-beta-upgrade@example.edu",
        )
        connection.executescript(
            """
            CREATE TABLE email_delivery_attempts (
                attempt_id VARCHAR(36) PRIMARY KEY,
                email_task_id INTEGER NOT NULL REFERENCES email_tasks(id) ON DELETE CASCADE,
                owner_role VARCHAR(16) NOT NULL,
                runtime_id VARCHAR(128) NOT NULL,
                owner_generation VARCHAR(128) NOT NULL,
                owner_pid INTEGER NOT NULL,
                outcome VARCHAR(48) NOT NULL DEFAULT 'claimed',
                claimed_at DATETIME NOT NULL,
                finalized_at DATETIME,
                smtp_accepted_at DATETIME,
                prepared_rfc_message_id VARCHAR(255),
            ...
            CREATE INDEX ix_email_delivery_attempts_task_claimed
                ON email_delivery_attempts(email_task_id, claimed_at);
            ...
            ALTER TABLE email_logs ADD COLUMN delivery_attempt_id VARCHAR(36);
            CREATE UNIQUE INDEX uq_email_logs_delivery_attempt_id
                ON email_logs(delivery_attempt_id)
        ...
        task_id = int(
            connection.execute(
                ...
                INSERT INTO email_tasks (
                    source, identity_id, llm_profile_id, professor_id, status,
                    approved_subject, approved_body_text, retry_count,
                    delivery_attempt_id, delivery_outcome, delivery_outcome_at,
                    created_at, updated_at
                ) VALUES (
                    'manual', ?, ?, ?, 'sent', 'Applied beta subject',
                    'Applied beta body', 1, 'applied-beta-attempt',
                    'smtp_accepted', '2026-08-10 09:00:01',
                    '2026-08-10 08:59:00', '2026-08-10 09:00:01'
                )
                """,
        ...
        connection.execute(
            ...
            INSERT INTO email_delivery_attempts (
                ...
                smtp_accepted_at, prepared_rfc_message_id, subject, content
            ) VALUES (
                'applied-beta-attempt', ?, 'worker', 'runtime-1',
                'generation-1', 1234, 'smtp_accepted',
                '2026-08-10 09:00:00', '2026-08-10 09:00:01',
                '2026-08-10 09:00:01', '<applied-beta@example.edu>',
                'Applied beta subject', 'Applied beta body'
            )
            """,
            (task_id,),
        )
        ...
        attempt = upgraded.execute(
            """
            SELECT id, email_task_id, identity_id, professor_id, attempt_number,
                   status, started_at, completed_at, outcome, owner_generation
            FROM email_delivery_attempts
            WHERE id = 'applied-beta-attempt'
            """,
        ).fetchone()
        columns = {
            row[1]
        ...
        delivery_log_foreign_keys = [
            row
            for row in upgraded.execute("PRAGMA foreign_key_list('email_logs')")
            if row[3] == "delivery_attempt_id"
        ]
        foreign_key_check = upgraded.execute("PRAGMA foreign_key_check").fetchall()
        version = upgraded.execute(
            "SELECT version_num FROM alembic_version",
        ...
        self.assertEqual(attempt["email_task_id"], task_id)
        self.assertEqual(attempt["identity_id"], identity_id)
        self.assertEqual(attempt["professor_id"], professor_id)
        self.assertEqual(attempt["attempt_number"], 1)
        self.assertEqual(attempt["status"], "accepted")
        self.assertEqual(attempt["outcome"], "smtp_accepted")
        self.assertEqual(attempt["owner_generation"], "generation-1")
        self.assertIn("id", columns)
        self.assertIn("started_at", columns)
        self.assertNotIn("attempt_id", columns)
        self.assertNotIn("claimed_at", columns)
        self.assertEqual(
            [(row[2], row[4]) for row in delivery_log_foreign_keys],
