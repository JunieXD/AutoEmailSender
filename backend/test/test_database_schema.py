...
class MigrationScriptTests(unittest.TestCase):
    ...
    def test_crawl_enrichment_operation_migration_preserves_existing_jobs(self) -> None:
        ...
        env = os.environ.copy()
        env["DATABASE_URL"] = f"sqlite+aiosqlite:///{database_path.as_posix()}"
        previous_revision = "20260811_delivery_reconcile"
        migration_revision = "20260817_crawl_enrichment_op"
        ...
        with sqlite3.connect(database_path) as connection:
            ...
            historical_job = connection.execute(
                """
                SELECT university, active_candidate_enrichment_operation_id
                FROM crawl_jobs
                """
            ).fetchone()
        self.assertIn("active_candidate_enrichment_operation_id", columns)
        self.assertEqual(historical_job, ("历史大学", None))
