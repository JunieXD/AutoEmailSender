...
from app.modules.crawler.runtime.models import CrawlerClaimedWork, CrawlerWorkKind
from app.services.runtime_manager import RuntimeManager


class CrawlerRuntimeRuntimeRoutingTests(unittest.IsolatedAsyncioTestCase):
            ...
            async with session_factory() as session:
                ...
                tasks = list(
                    await session.scalars(
                        ...
                        .where(CrawlPageTask.job_id == job.id)
                        .order_by(CrawlPageTask.id)
                    )
                )
            self.assertEqual(
                [task.normalized_url for task in tasks],
                ["https://example.edu/faculty", "https://example.edu/page2"],
            )
        finally:
    ...
    async def test_runtime_runtime_dispatches_claimed_chunk_worker(self) -> None:
        fd, db_path = tempfile.mkstemp(suffix=".db")
        os.close(fd)
        engine = create_async_engine(f"sqlite+aiosqlite:///{Path(db_path).as_posix()}")
        try:
            ...
            from app.modules.crawler.runtime.scheduler import run_crawler_once

            processed = await run_crawler_once(session_factory, worker_id="w1")
    ...
    async def test_runtime_runtime_uses_unique_owner_for_each_claim(self) -> None:
        session_factory = Mock()
        claimed = CrawlerClaimedWork(
        ...
        with (
            patch(
                "app.modules.crawler.runtime.scheduler.uuid.uuid4",
            ) as mocked_uuid,
            patch(
                "app.modules.crawler.runtime.scheduler.claim_next_work",
                new=AsyncMock(return_value=claimed),
            ) as mocked_claim,
            patch(
                "app.modules.crawler.runtime.page_worker.run_crawler_page_worker_once",
            ...
            processed = await run_crawler_once(
                session_factory, worker_id="crawler-worker-1"
            )
    ...
    async def test_runtime_manager_uses_runtime_worker_entry(self) -> None:
        session = object()
        ...
        with patch("app.services.runtime_manager.get_settings") as mocked_get_settings:
            ...
            with (
                ...
                patch.object(
                    manager, "_loop", new=Mock(side_effect=build_idle_loop)
                ) as mocked_loop,
            ):
                await manager.start()
        ...
        self.assertEqual(crawler_worker.func.__name__, "run_crawler_once")
