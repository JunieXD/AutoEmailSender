...
class SchemaBackupTests(unittest.TestCase):
    def test_creates_database_copy_and_metadata_file(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                self.assertEqual(
                    copied.execute("SELECT id FROM sample").fetchone()[0], 1
                )
            finally:
                copied.close()
    ...
    def test_creates_consistent_backup_while_source_connection_is_open(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                connection.commit()
                connection.execute(
                    "INSERT INTO sample (id, name) VALUES (2, 'uncommitted')"
                )
    ...
    def test_prunes_schema_backups_to_recent_five_pairs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for index in range(7):
                db_file = (
                    backup_dir
                    / f"auto_email_sender.before-2.3.0.20260605-12000{index}.db"
                )
            ...
            self.assertNotIn(
                "auto_email_sender.before-2.3.0.20260605-120000.db", remaining
            )
            self.assertNotIn(
                "auto_email_sender.before-2.3.0.20260605-120001.db", remaining
            )
