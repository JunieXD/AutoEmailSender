...
from app.modules.crawler.runtime.models import CrawlerWorkKind, CrawlerWorkerConfig
from test.schema_database import create_schema_sqlite_database
from app.modules.crawler.runtime.scheduler import (
    ZERO_CANDIDATE_BROWSER_RETRY_REASON,
    _run_claimed_work_with_heartbeat,
    claim_next_work,
    ensure_job_active,
    run_crawler_scheduler_once,
)
from app.modules.crawler.runtime.lease import CrawlerClaimFence


class CrawlerRuntimeSchedulerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_default_chunk_concurrency_is_three(self) -> None:
        self.assertEqual(CrawlerWorkerConfig().chunk_concurrency, 3)
    ...
    async def test_default_job_concurrency_keeps_later_job_queued(self) -> None:
        ...
        first = await claim_next_work(self.session_factory, worker_id="w1")
        second = await claim_next_work(self.session_factory, worker_id="w2")

        self.assertEqual(first.job_id, first_job_id)
        self.assertEqual(second.kind, CrawlerWorkKind.IDLE)
        async with self.session_factory() as session:
    ...
    async def test_single_job_can_use_multiple_internal_page_workers(self) -> None:
        ...
        self.assertEqual(first.kind, CrawlerWorkKind.PAGE)
        self.assertEqual(second.kind, CrawlerWorkKind.PAGE)
    ...
    async def test_additional_claim_does_not_reset_run_active_start(self) -> None:
        ...
        await claim_next_work(self.session_factory, worker_id="w1")
        async with self.session_factory() as session:
        ...
        await claim_next_work(self.session_factory, worker_id="w2")
    ...
    async def test_active_jobs_rotate_before_reusing_stage_priority(self) -> None:
        ...
        self.assertEqual(first.job_id, first_job_id)
        self.assertEqual(first.kind, CrawlerWorkKind.CHUNK)
        self.assertEqual(second.job_id, second_job_id)
        self.assertEqual(second.kind, CrawlerWorkKind.PAGE)
    ...
    async def test_claims_chunk_before_page_and_enrichment(self) -> None:
        ...
        async with self.session_factory() as session:
            ...
            await session.flush()
            session.add(
                CrawlCandidateEnrichmentTask(job_id=job_id, candidate_id=candidate.id)
            )
            await session.commit()

        claimed = await claim_next_work(
            self.session_factory, worker_id="w1", config=CrawlerWorkerConfig()
        )

        self.assertEqual(claimed.kind, CrawlerWorkKind.CHUNK)
        async with self.session_factory() as session:
    ...
    async def test_does_not_claim_new_page_while_chunk_is_pending(self) -> None:
        ...
        async with self.session_factory() as session:
            session.add(
                CrawlPageTask(
                    job_id=job_id,
                    normalized_url="https://example.edu/page2",
                    original_url="https://example.edu/page2",
                )
            )
            session.add(
                CrawlPageChunk(
    ...
    async def test_does_not_claim_when_job_paused(self) -> None:
        ...
        self.assertEqual(claimed.kind, CrawlerWorkKind.IDLE)
    ...
    async def test_claiming_work_marks_queued_job_and_run_running(self) -> None:
        ...
        async with self.session_factory() as session:
            ...
            run = CrawlJobRun(
                job_id=job_id, attempt_number=1, status=CrawlJobStatus.QUEUED.value
            )
            session.add(run)
            await session.flush()
        ...
        self.assertEqual(claimed.kind, CrawlerWorkKind.PAGE)
        async with self.session_factory() as session:
            job = await session.get(CrawlJob, job_id)
    ...
    async def test_expired_processing_page_can_be_reclaimed(self) -> None:
        ...
        claimed = await claim_next_work(
            self.session_factory, worker_id="new", config=CrawlerWorkerConfig()
        )
    ...
    async def test_runtime_concurrency_reduction_demotes_overflow_job(self) -> None:
        ...
        claimed = await claim_next_work(self.session_factory, worker_id="w1")
    ...
    async def test_heartbeat_renews_claim_during_long_work(self) -> None:
        ...
        async with self.session_factory() as session:
            ...
            await session.commit()
        config = CrawlerWorkerConfig(lease_seconds=1)
        claimed = await claim_next_work(
            self.session_factory,
    ...
    async def test_claim_uses_runtime_enrichment_concurrency_setting(self) -> None:
        ...
        async with self.session_factory() as session:
            ...
            session.add(
                AppSetting(
                    crawler_profile_enrichment_concurrency=1, crawler_host_concurrency=1
                )
            )
            first = CrawlCandidate(
                job_id=job_id, name="张三", profile_url="https://example.edu/a"
            )
            second = CrawlCandidate(
                job_id=job_id, name="李四", profile_url="https://example.edu/b"
            )
    ...
    async def test_claim_respects_runtime_host_concurrency_for_enrichment(self) -> None:
        ...
        async with self.session_factory() as session:
            ...
            session.add(
                AppSetting(
                    crawler_profile_enrichment_concurrency=3, crawler_host_concurrency=1
                )
            )
            active = CrawlCandidate(
                job_id=job_id, name="张三", profile_url="https://same.example.edu/a"
            )
            same_host = CrawlCandidate(
                job_id=job_id, name="李四", profile_url="https://same.example.edu/b"
            )
            other_host = CrawlCandidate(
                job_id=job_id, name="王五", profile_url="https://other.example.edu/c"
            )
            ...
            session.add_all(
                [
                    ...
                    CrawlCandidateEnrichmentTask(
                        job_id=job_id,
                        candidate_id=same_host.id,
                        status=CrawlCandidateEnrichmentTaskStatus.PENDING.value,
                    ),
                    CrawlCandidateEnrichmentTask(
                        job_id=job_id,
                        candidate_id=other_host.id,
                        status=CrawlCandidateEnrichmentTaskStatus.PENDING.value,
                    ),
                ]
            )
        ...
        self.assertEqual(claimed.kind, CrawlerWorkKind.ENRICHMENT)
        async with self.session_factory() as session:
            claimed_task = await session.get(
                CrawlCandidateEnrichmentTask, claimed.work_item_id
            )
        assert claimed_task is not None
        self.assertEqual(claimed_task.candidate_id, other_id)

    async def test_scheduler_marks_job_failed_when_no_work_and_no_candidates_remain(
        self,
    ) -> None:
        job_id = await self._create_job()

        processed = await run_crawler_scheduler_once(
            self.session_factory, worker_id="w1"
        )
    ...
    async def test_scheduler_requeues_direct_list_entry_once_when_no_candidates(
        self,
    ) -> None:
        job_id = await self._create_job()
    ...
    async def test_scheduler_does_not_requeue_entry_after_browser_was_attempted(
        self,
    ) -> None:
        job_id = await self._create_job()
        url = "https://example.edu"
        ...
        await run_crawler_scheduler_once(self.session_factory, worker_id="scheduler")
    ...
    async def test_scheduler_does_not_requeue_profile_entry_for_zero_candidates(
        self,
    ) -> None:
    ...
    async def test_scheduler_marks_job_needs_review_when_candidates_exist_and_no_work_remains(
        self,
    ) -> None:
        job_id = await self._create_job()
        async with self.session_factory() as session:
    ...
    async def test_scheduler_records_enrichment_completion_event_when_queue_finishes(
        self,
    ) -> None:
        job_id = await self._create_job()
        async with self.session_factory() as session:
            ...
            candidate_a = CrawlCandidate(
                job_id=job_id, name="张三", profile_url="https://example.edu/a"
            )
            candidate_b = CrawlCandidate(
                job_id=job_id, name="李四", profile_url="https://example.edu/b"
            )
            candidate_c = CrawlCandidate(
                job_id=job_id, name="王五", profile_url="https://example.edu/c"
            )
        ...
        processed = await run_crawler_scheduler_once(
            self.session_factory, worker_id="scheduler"
        )
        ...
        self.assertEqual(
            terminal_event["raw"]["operation_id"], "enrichment-operation-1"
        )
    ...
    async def test_scheduler_enrichment_completion_event_counts_current_run_only(
        self,
    ) -> None:
        job_id = await self._create_job()
        ...
        async with self.session_factory() as session:
            ...
            await session.flush()
            job.current_run_id = run.id
            old_candidate = CrawlCandidate(
                job_id=job_id, name="旧候选", profile_url="https://example.edu/old"
            )
            current_candidate = CrawlCandidate(
                job_id=job_id, name="新候选", profile_url="https://example.edu/new"
            )
    ...
    async def test_scheduler_counts_merged_alias_tasks_as_one_candidate(self) -> None:
        ...
        await run_crawler_scheduler_once(
            self.session_factory,
            worker_id="scheduler",
        )
    ...
    async def test_scheduler_does_not_finalize_job_with_active_processing_page(
        self,
    ) -> None:
        job_id = await self._create_job()
        active_until = datetime.now(UTC) + timedelta(minutes=5)
    ...
    async def test_scheduler_does_not_finalize_job_with_active_processing_chunk(
        self,
    ) -> None:
        job_id = await self._create_job()
        active_until = datetime.now(UTC) + timedelta(minutes=5)
    ...
    async def test_scheduler_does_not_finalize_job_with_active_processing_enrichment(
        self,
    ) -> None:
        job_id = await self._create_job()
        active_until = datetime.now(UTC) + timedelta(minutes=5)
    ...
    async def test_scheduler_does_not_finalize_job_with_expired_reclaimable_page(
        self,
    ) -> None:
        job_id = await self._create_job()
        ...
        processed = await run_crawler_scheduler_once(
            self.session_factory,
            worker_id="w1",
            config=CrawlerWorkerConfig(page_concurrency=1),
        )
    ...
    async def test_scheduler_finishes_current_run_when_job_becomes_reviewable(
        self,
    ) -> None:
        job_id = await self._create_job()
        async with self.session_factory() as session:
    ...
    async def test_terminal_failures_with_candidates_mark_needs_review_when_no_retryable_work(
        self,
    ) -> None:
        job_id = await self._create_job()
        async with self.session_factory() as session:
    ...
    async def test_profile_job_with_all_terminal_pages_and_no_candidates_fails(
        self,
    ) -> None:
        job_id = await self._create_job(entry_type="profile")
        async with self.session_factory() as session:
    ...
    async def test_no_candidates_with_terminal_chunk_failure_uses_terminal_error_message(
        self,
    ) -> None:
        job_id = await self._create_job()
        async with self.session_factory() as session:
    ...
    async def test_terminal_connection_error_is_adapted_for_final_job_message(
        self,
    ) -> None:
        job_id = await self._create_job()
        async with self.session_factory() as session:
    ...
    async def test_profile_job_with_candidate_enters_needs_review_after_terminal_pages(
        self,
    ) -> None:
        job_id = await self._create_job(entry_type="profile")
        async with self.session_factory() as session:
            session.add(
                CrawlCandidate(
                    job_id=job_id,
                    name="张三",
                    profile_url="https://example.edu/teacher/a",
                )
            )
            session.add_all(
                [
        ...
        async with self.session_factory() as session:
            session.add(
                CrawlPageTask(
                    ...
                    normalized_url="https://example.edu/a",
                    original_url="https://example.edu/a",
                    status=CrawlPageTaskStatus.FAILED_RETRYABLE.value,
                )
            )
            await session.commit()
    ...
    async def test_retryable_page_in_backoff_is_not_claimed_until_delay_expires(
        self,
    ) -> None:
        job_id = await self._create_job()
        ...
        async with self.session_factory() as session:
            session.add(
                CrawlPageTask(
                    ...
                    original_url="https://example.edu/a",
                    status=CrawlPageTaskStatus.FAILED_RETRYABLE.value,
                    lease_expires_at=delayed_until,
                )
            )
            await session.commit()
    ...
    async def test_scheduler_does_not_finalize_job_with_retryable_page_in_backoff(
        self,
    ) -> None:
        job_id = await self._create_job()
        delayed_until = datetime.now(UTC) + timedelta(seconds=30)
    ...
    async def test_claim_skips_row_when_conditional_update_loses_race(self) -> None:
        ...
        with patch(
            "app.modules.crawler.runtime.scheduler._conditional_claim_page_task",
            return_value=LostRaceResult(),
        ):
            claimed = await claim_next_work(
                self.session_factory, worker_id="w1", config=CrawlerWorkerConfig()
            )
    ...
    async def _create_job(
        self, *, status: str = CrawlJobStatus.RUNNING.value, entry_type: str = "list"
    ) -> int:
        async with self.session_factory() as session:
            job = CrawlJob(
                university="示例大学",
