...
from app.modules.crawler.runtime.routing import (
    ...
    build_entry_routing_prompt,
    build_pagination_routing_prompt,
...
class CrawlerRuntimeRoutingTests(unittest.IsolatedAsyncioTestCase):
    async def test_routing_phase_uses_shared_structured_output_request(self) -> None:
        ...
        with patch(
            "app.modules.crawler.runtime.routing.request_crawler_structured_completion",
    ...
    def test_routing_prompts_keep_entry_selection_and_pagination_separate(self) -> None:
        ...
        entry_prompt = build_entry_routing_prompt(
            university="示例大学",
            school="计算机学院",
            source_url="https://example.edu/list",
            routing_context=context,
        )
        pagination_prompt = build_pagination_routing_prompt(
            university="示例大学",
            school="计算机学院",
            source_url="https://example.edu/list",
            routing_context=context,
        ...
        self.assertIn("仍检查它是否只是总名单的一个明确部分", entry_prompt)
        self.assertIn("并列人员分类", entry_prompt)
        self.assertIn("分类的前后页、页码不是分类", entry_prompt)
        self.assertIn("人员类别或单位内部组织", entry_prompt)
        self.assertIn("不要把同一批人按另一种维度再抓一遍", entry_prompt)
        self.assertIn("按表彰或项目选出的少数人", entry_prompt)
    ...
    def test_pagination_payload_requires_boolean_to_match_urls(self) -> None:
        PaginationRoutingPayload.model_validate(
            {
    ...
    async def test_entry_page_runs_entry_selection_then_pagination(self) -> None:
        ...
        with patch(
            "app.modules.crawler.runtime.routing._invoke_structured_routing_phase",
            new=phase_mock,
        ):
            result = await invoke_page_routing_agent(
                object(),
                session_factory=object(),
                university="示例大学",
                school="计算机学院",
