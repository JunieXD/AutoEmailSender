...
class ContactStatusTests(unittest.TestCase):
    ...
    def test_successful_sent_log_keeps_professor_contacted_after_later_preparing_task(
        self,
    ) -> None:
        async def scenario():
    ...
    def test_empty_failure_summary_sent_log_counts_as_successful(self) -> None:
        async def scenario():
            async with self.session_factory() as session:
                ...
                llm_profile = self._build_llm_profile()
                professor = Professor(
                    name="空失败信息老师", email="empty-failure@example.edu"
                )
                session.add_all([identity, llm_profile, professor])
                await session.flush()
                session.add(
    ...
    def test_received_log_without_task_marks_professor_replied(self) -> None:
        async def scenario():
            async with self.session_factory() as session:
                ...
                llm_profile = self._build_llm_profile()
                professor = Professor(
                    name="无任务回复老师", email="reply-only@example.edu"
                )
                session.add_all([identity, llm_profile, professor])
                await session.flush()
                session.add(
    ...
    def test_sent_log_without_task_marks_professor_contacted(self) -> None:
        async def scenario():
            async with self.session_factory() as session:
                ...
                llm_profile = self._build_llm_profile()
                professor = Professor(
                    name="无任务发送老师", email="sent-only@example.edu"
                )
                session.add_all([identity, llm_profile, professor])
                await session.flush()
                session.add(
        ...
        async def scenario():
            ...
            class ProjectionOnlySession:
                async def execute(self, statement):
                    ...
                    test_case.assertEqual(
                        tuple(statement._group_by_clauses), (EmailLog.professor_id,)
                    )
                    return [
                        (
                            1,
                ...
                async def scalars(self, statement):
                    raise AssertionError(
                        "EmailLog status query should use projected columns"
                    )
    ...
    def test_dashboard_professor_list_keeps_large_task_columns_unloaded(self) -> None:
        async def scenario():
            async with self.session_factory() as session:
                ...
                llm_profile = self._build_llm_profile()
                professor = Professor(
                    name="窄列任务老师", email="narrow-task@example.edu"
                )
                session.add_all([identity, llm_profile, professor])
                await session.flush()
                task = EmailTask(
            ...
            async with self.session_factory() as session:
                ...
                tasks_by_professor = build_status.await_args.kwargs[
                    "tasks_by_professor"
                ]
    ...
    def test_dashboard_professor_list_reuses_loaded_tasks_for_contact_status(
        self,
    ) -> None:
        async def scenario():
            async with self.session_factory() as session:
                ...
                llm_profile = self._build_llm_profile()
                professor = Professor(
                    name="复用任务老师", email="reuse-task@example.edu"
                )
                session.add_all([identity, llm_profile, professor])
                await session.flush()
                task = EmailTask(
        ...
        self.assertEqual(
            [task.id for task in tasks_by_professor[items[0].id]], [task_id]
        )
