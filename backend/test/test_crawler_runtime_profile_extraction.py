...
from app.modules.crawler.runtime.profile_extraction import (
    ProfileExtractionPayload,
    build_profile_extraction_prompt,
    invoke_profile_extraction_agent,
)
...
class CrawlerRuntimeProfileExtractionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_invoke_does_not_issue_repair_call_for_invalid_output(self) -> None:
        ...
        with patch(
            "app.modules.crawler.runtime.profile_extraction.request_crawler_structured_completion",
            new=AsyncMock(side_effect=LLMRuntimeError("模型返回的 JSON 结构无效")),
        ) as request_mock:
            with self.assertRaisesRegex(LLMRuntimeError, "JSON 结构无效"):
                await invoke_profile_extraction_agent(
                    llm_profile,
    ...
    async def test_invoke_uses_shared_structured_output_request(self) -> None:
        ...
        wire_payload = CrawlerProfileExtractionWirePayload(
            status="no_candidate",
    ...
    async def test_payload_accepts_no_candidate(self) -> None:
        payload = ProfileExtractionPayload.model_validate(
