...
from app.modules.crawler.runtime.retry import (
    MAX_CRAWLER_ATTEMPTS,
    MAX_CRAWLER_CONNECTIVITY_ATTEMPTS,
    mark_crawler_failed,
...
class CrawlerRuntimeRetryTests(unittest.TestCase):
    ...
    def test_fourth_failed_attempt_becomes_terminal_without_backoff(self) -> None:
        item = FakeWorkItem(
            attempt_count=MAX_CRAWLER_ATTEMPTS,
            failure_count=MAX_CRAWLER_ATTEMPTS - 1,
        )
    ...
    def test_connectivity_errors_receive_extended_failure_budget(self) -> None:
        ...
        item = FakeWorkItem(
            attempt_count=30,
            failure_count=MAX_CRAWLER_CONNECTIVITY_ATTEMPTS - 2,
        )
        ...
        self.assertEqual(
            item.failure_count,
            MAX_CRAWLER_CONNECTIVITY_ATTEMPTS - 1,
        )
