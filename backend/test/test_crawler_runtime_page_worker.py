...
from app.modules.crawler.runtime.scheduler import ZERO_CANDIDATE_BROWSER_RETRY_REASON


class CrawlerRuntimePageWorkerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_profile_extraction_adaptation_cache_is_committed_before_session_closes(
        ...
        with (
            ...
            patch(
                "app.modules.crawler.runtime.page_worker.ensure_llm_runtime_adaptation",
                new=AsyncMock(side_effect=fake_ensure),
            ),
            patch(
                "app.modules.crawler.runtime.page_worker.invoke_profile_extraction_agent",
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.routing_patcher = patch(
            "app.modules.crawler.runtime.page_worker.invoke_page_routing_agent",
            new=AsyncMock(
                return_value=PageRoutingResult(
                    discovered_urls=[],
        ...
        self.browser_patcher = patch(
            "app.modules.crawler.runtime.page_worker.fetch_page_browser",
            new=AsyncMock(
                return_value=PageSnapshot(
                    url="https://example.edu/faculty",
                    fetch_method="browser",
                    status="failed",
                    error_message="browser unavailable in unit test",
                )
            ),
        )
    ...
    async def test_spa_route_uses_browser_without_direct_fetch(self) -> None:
        ...
        with (
            ...
            patch(
                "app.modules.crawler.runtime.page_worker.fetch_page_direct", new=direct_mock
            ),
        ):
            processed = await run_crawler_page_worker_once(
                self.session_factory,
    ...
    async def test_fetch_page_browser_accepts_profile_intent(self) -> None:
        ...
        with patch(
            "app.modules.crawler.runtime.page_worker.browser_investigate",
    ...
    async def test_profile_entry_writes_debug_events_without_page_text(self) -> None:
        ...
        with (
            ...
            patch(
                "app.modules.crawler.runtime.page_worker.append_crawler_worker_debug_event"
            ) as debug_mock,
        ):
            await run_crawler_page_worker_once(
                self.session_factory, task_id=task_id, worker_id="w1"
    ...
    async def test_page_worker_writes_runtime_debug_jsonl(self) -> None:
        job_id, task_id = await self._seed_page_task()
        snapshot = PageSnapshot(
            url="https://example.edu/faculty",
            status="succeeded",
    ...
    async def test_static_directory_keeps_useful_direct_snapshot(self) -> None:
        ...
        browser = PageSnapshot(
            url="https://example.edu/faculty",
            text="张三 李四",
            html="<a href='/zhang'>张三</a><a href='/li'>李四</a>",
            fetch_method="browser",
            status="succeeded",
        )
    ...
    async def test_same_job_same_domain_uses_browser_after_prior_direct_fallback(
        ...
        with (
            patch(
                "app.modules.crawler.runtime.page_worker.fetch_page_direct", new=AsyncMock()
            ) as direct_mock,
            patch(
                "app.modules.crawler.runtime.page_worker.fetch_page_browser",
                new=AsyncMock(return_value=browser),
    ...
    async def test_fetch_modes_use_distinct_underlying_paths(self) -> None:
        ...
        with (
            patch(
                "app.modules.crawler.runtime.page_worker.crawl_page_with_http",
