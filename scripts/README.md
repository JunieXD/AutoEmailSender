...
On the configured project Mac, `quality/run-windows-vm-release-qa.sh` transfers the committed `HEAD` to the dedicated Parallels Windows 11 VM and invokes `quality/run-windows-release-qa.ps1`. The default mode performs full pre-release packaging and lifecycle checks; `--quick` runs only cache-aware development checks and cannot satisfy release preflight. See `docs/operations/windows-parallels-release-qa.md`.
