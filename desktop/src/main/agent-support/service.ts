...
import {
  ...
  broadcastWindowsEnvironmentChangeSafely,
  ensureMacPath,
  ensureWindowsPath,
  hasPathEntry,
  macPathIsConfigured,
  ...
  removeMacPathBlock,
  removePathEntry,
  synchronizeWindowsProcessPath,
  writeWindowsUserPath,
} from "./environment-path.js";
import {
  directoryFingerprintMatches,
  fileTextMatches,
  isSha256,
  sha256Directory,
} from "./fingerprints.js";
import {
  commitManagedPathChanges,
  ManagedPathChange,
  pathExists,
  replaceManagedPath,
  rollbackManagedPathChanges,
  stageManagedPathRemoval,
...
export function resolveAgentSupportPaths(
  ...
  return {
    ...
    shellProfilePath:
      options.platform === "darwin"
        ? path.join(options.homePath, ".zshrc")
        : null,
    commandDirectory,
  };
}
...
export function createAgentSupportService(options: AgentSupportServiceOptions) {
  ...
  const broadcastWindowsChange =
    options.broadcastWindowsEnvironmentChange ??
    broadcastWindowsEnvironmentChange;
  const detectAgentInstallation =
    options.detectAgentInstallation ??
  ...
  const getStatus = async (): Promise<AgentSupportStatus> => {
    ...
    if (!installationHealthy) {
      const hasSkillUpdate = agents.some(
        (agent) => agent.state === "needs_update",
      );
      return buildStatus(
        "needs_repair",
        hasSkillUpdate
    ...
    const hasInstalledAgent = agents.some(
      (agent) => agent.state === "installed",
    );
    return buildStatus(
      "enabled",
      hasInstalledAgent
  ...
  const enable = async (
    request: AgentSupportEnableOptions = {},
  ...
  const installAgentSkill = async (
    ...
    const replacement = await installSkill(
      paths.skillSource,
      target,
      isManaged,
    );
  ...
  const uninstallAgentSkill = async (
    agentId: AgentIntegrationId,
  ): Promise<AgentSupportStatus> => {
    const agent = getAgentDefinition(agentId);
    const previousManifest = await readManifest(paths.manifestPath);
    ...
    const agentIds = getManagedAgentIds(paths, previousManifest).filter(
      (managedAgentId) => managedAgentId !== agent.id,
    );
    await writeEnabledManifest(previousManifest.path_managed, agentIds);
    return getStatus();
  };
  ...
  const disable = async (): Promise<AgentSupportStatus> => {
    ...
    if (manifest?.enabled) {
      for (const agentId of getManagedAgentIds(paths, manifest)) {
        await rm(paths.agentSkillTargets[agentId], {
          recursive: true,
          force: true,
        });
      }
    ...
    if (manifest?.enabled && manifest.path_managed) {
      if (options.platform === "darwin" && paths.shellProfilePath !== null) {
        ...
        const updatedPath = removePathEntry(
          currentPath,
  ...
  const dismissOnboarding = async (): Promise<AgentSupportStatus> => {
    ...
    if (current?.enabled) {
      await writeEnabledManifest(
        current.path_managed,
        getManagedAgentIds(paths, current),
      );
    } else {
      await writeDisabledManifest(true);
    }
  ...
  const synchronize = async (): Promise<AgentSupportStatus> => {
    ...
    if (
      await isInstallationHealthy({ options, paths, manifest, readWindowsPath })
    ) {
  ...
  async function installCliSupport(
  ...
  async function installManagedSupport(
    previousManifest: AgentSupportManifest,
  ): Promise<AgentSupportStatus> {
    return installSupport(
      previousManifest,
      getManagedAgentIds(paths, previousManifest),
    );
  }
    ...
    try {
      for (const agentId of agentIds) {
        replacements.push(
          await installSkill(
            paths.skillSource,
            paths.agentSkillTargets[agentId],
            true,
          ),
        );
      }
      const pathManaged =
        options.platform === "darwin"
          ...
          : await ensureWindowsPath(
              paths.commandDirectory,
              Boolean(
                previousManifest?.enabled && previousManifest.path_managed,
              ),
              readWindowsPath,
              writeWindowsPath,
              processEnvironment,
...
async function hasRequiredSourceFiles(
  paths: AgentSupportPaths,
): Promise<boolean> {
  return (
    (await pathExists(paths.cliSource)) &&
...
async function installCli(
  ...
  try {
    if (platform === "darwin") {
      ...
      await writeFile(
        temporaryPath,
        windowsCliLauncherContent(paths.cliSource),
        {
          encoding: "utf8",
          flag: "wx",
        },
...
async function isInstallationHealthy(input: {
  ...
  if (
    ...
    !(await directoryFingerprintMatches(
      paths.cliBundleSource,
      manifest.cli_sha256,
    ))
  ) {
    return false;
  ...
  try {
    return (
      (await lstat(paths.cliTarget)).isFile() &&
      (await fileTextMatches(
        paths.cliTarget,
        windowsCliLauncherContent(paths.cliSource),
      )) &&
...
async function getAgentStatuses(
  ...
  const sourceSha256 = await sha256Directory(paths.skillSource).catch(
    () => null,
  );
...
async function isManagedAgentSkillHealthy(
  ...
  return (
    (await directoryFingerprintMatches(target, record.skill_sha256)) &&
...
async function findUnmanagedCliConflict(
  ...
  for (const candidate of new Set([paths.cliTarget, paths.legacyCliTarget])) {
    if (
      ...
      path.resolve(candidate) !== path.resolve(managedTarget ?? "")
    ) {
      return candidate;
    }
  }
...
function getMissingAgentSupportFilesMessage(
  options: AgentSupportServiceOptions,
): string {
...
async function readManifest(
  manifestPath: string,
): Promise<AgentSupportManifest | null> {
  try {
    ...
    return {
      ...
      cli_source:
        typeof value.cli_source === "string" ? value.cli_source : null,
      skill_source:
        typeof value.skill_source === "string" ? value.skill_source : null,
...
function readLegacyManagedAgents(
  value: Record<string, unknown>,
): AgentSupportManifest["agents"] {
...
function getAgentDefinition(agentId: AgentIntegrationId): AgentDefinition {
  const agent = AGENT_INTEGRATIONS.find(
...
async function detectInstalledAgent(
  ...
  if (options.platform === "win32") {
    ...
    const systemRoot =
      options.processEnvironment?.SystemRoot ??
      process.env.SystemRoot ??
      "C:\\Windows";
    return commandSucceeds(path.join(systemRoot, "System32", "where.exe"), [
      "codex",
    ]);
  }
  if (options.platform === "darwin") {
